...
apply_minimal_theme() {
    ...
    # Status right with compact spacing
    local status_right="\
#[fg=$inactive_color]│ #[fg=$accent_color] #[fg=$text_color]#(cd #{pane_current_path} && \
...
#[fg=$inactive_color] │ #[fg=$accent_color] #[fg=$text_color]#(free | awk '/^Mem/ { printf(\"%.0f%%\", \$3/\$2 * 100 - 0.5) }')\
#[fg=$inactive_color] │ #[fg=$accent_color] #[fg=$text_color]#(date +%d/%m)\
#[fg=$inactive_color] │ #[fg=$accent_color] #[fg=$text_color]#(date +%H:%M)\
#[fg=$inactive_color] │ #(~/.tmux/plugins/tmux-minimal-theme/scripts/battery_widget.sh) "
